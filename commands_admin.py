...
async def admin_grant_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with session_scope() as session:
        user_row = session.execute(text("SELECT id FROM users WHERE telegram_id = :tg"), {"tg": target_tg}).mappings().first()
        if not user_row:
    ...
    await update.effective_message.reply_text(f"Granted {days} day(s) trial to {target_tg}, expires {new_expiry.isoformat()} (UTC).")
...
async def admin_trial_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with session_scope() as session:
        trial_row = session.execute(
            text("SELECT provider_sub_id, created_at FROM subscriptions WHERE provider = 'trial' AND user_id = (SELECT id FROM users WHERE telegram_id = :tg) ORDER BY created_at DESC LIMIT 1"),
            {"tg": target_tg}
        ).mappings().first()
    if not trial_row:
        await update.effective_message.reply_text("No trial found for that user.")
        return
    await update.effective_message.reply_text(f"User {target_tg} — trial expires: {trial_row['provider_sub_id']} — created: {trial_row['created_at']}")
