...
def paypal_upgrade_url_for(tg_id: str | None):
    return None
...
def _trial_status_line_for(tg_id: str | None) -> str:
    ...
    with session_scope() as session:
        row = session.execute(text("SELECT s.provider_sub_id FROM subscriptions s JOIN users u ON u.id=s.user_id WHERE u.telegram_id=:tg AND s.provider='trial' ORDER BY s.created_at DESC LIMIT 1"), {"tg": tg_id}).mappings().first()
        if not row or not row.get("provider_sub_id"):
            return "Trial: no active trial — contact admin"
        try:
            from datetime import datetime, timezone
            expiry = datetime.fromisoformat(row.get("provider_sub_id"))
