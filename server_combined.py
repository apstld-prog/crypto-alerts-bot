...
# Off-Binance tokens: /listalts, /listpresales, /alts <SYMBOL>, and /price fallback
# FIXES: commit on deletions + handle inline ack:keep/ack:del buttons
# NEW: Binance exchangeInfo cache for auto-detecting new USDT listings
...
# ──────────────────────────────────────────────────────────────────────────────
# Binance symbols cache (auto-detect new listings)
...
def resolve_symbol_auto(symbol: str | None) -> str | None:
    """Try existing resolve_symbol(); if not found, refresh Binance symbols and try again."""
    if not symbol:
        return None
    symbol = symbol.upper().strip()
    # 1) Existing mapping (worker_logic)
    pair = resolve_symbol(symbol)
    if pair:
        return pair
    # 2) Try cached/refresh Binance USDT pairs (auto-detect new listings)
    _refresh_binance_symbols()
    pair = _BINANCE_SYMBOLS.get(symbol)
    if pair:
        return pair
    # One more attempt with a forced refresh (first-time after listing)
...
def paypal_start(tg: str | None = Query(None), plan_id: str | None = Query(None)):
    ...
    if not target: return JSONResponse({"error": "No PAYPAL_SUBSCRIBE_URL and no plan_id"}, status_code=400)
    try:
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_html = (
        ...
        "• <code>/myalerts</code> • <code>/delalert &lt;id&gt;</code> • <code>/clearalerts</code> (Premium)\n"
        "• <code>/cancel_autorenew</code> • <code>/support &lt;message&gt;</code> • <code>/whoami</code>\n"
        "• <code>/alts &lt;SYMBOL&gt;</code> → notes & links for curated tokens\n"
        "• <code>/listalts</code> • <code>/listpresales</code>\n\n"
        "<b>Extras:</b> /feargreed /funding /topgainers /toplosers /chart /news /dca /pumplive\n"
    )
...
async def cmd_setalert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pair:
        await target_msg(update).reply_text("Unknown symbol. Try BTC, ETH, SOL … or check <code>/alts SYMBOL</code>.", parse_mode=ParseMode.HTML); return
    tg_id = str(update.effective_user.id)
...
async def cmd_alts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show only curated note & links for a token (no price)."""
    if not context.args:
...
# --- Support / billing / admin (ίδια όπως στην προηγούμενη έκδοση σου) ---
...
# Admin helpers/texts (ίδιο περιεχόμενο με την τελευταία σου working έκδοση)
ADMIN_HELP = (
    "Admin Commands\n\n"
    "• /adminstats • /adminsubs • /admincheck • /listalerts\n"
    "• /runalerts • /resetalert <id> • /testalert\n"
)
def is_admin_id(tg_id: str) -> bool: return tg_id in _ADMIN_IDS
def _require_admin(update: Update) -> bool: return not is_admin_id(str(update.effective_user.id))

async def cmd_adminhelp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if _require_admin(update): await target_msg(update).reply_text("Admins only."); return
...
async def cmd_adminstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    subs_total = subs_active = subs_cancel = subs_cancelled = subs_unknown = 0
    note = ""
    with session_scope() as s:
        try: users_total = s.execute(text("SELECT COUNT(*) FROM users")).scalar_one()
        except Exception as e: note += f"\n• users: {e}"
        try: users_premium = s.execute(text("SELECT COUNT(*) FROM users WHERE is_premium=TRUE")).scalar_one()
        except Exception as e: note += f"\n• users_premium: {e}"
        try:
            alerts_total = s.execute(text("SELECT COUNT(*) FROM alerts")).scalar_one()
            alerts_active = s.execute(text("SELECT COUNT(*) FROM alerts WHERE enabled=TRUE")).scalar_one()
        except Exception as e: note += f"\n• alerts: {e}"
        try:
            subs_total = s.execute(text("SELECT COUNT(*) FROM subscriptions")).scalar_one()
            subs_active = s.execute(text("SELECT COUNT(*) FROM subscriptions WHERE status_internal='ACTIVE'")).scalar_one()
            subs_cancel = s.execute(text("SELECT COUNT(*) FROM subscriptions WHERE status_internal='CANCEL_AT_PERIOD_END'")).scalar_one()
            subs_cancelled = s.execute(text("SELECT COUNT(*) FROM subscriptions WHERE status_internal='CANCELLED'")).scalar_one()
            subs_unknown = subs_total - subs_active - subs_cancel - subs_cancelled
        except Exception as e: note += f"\n• subs: {e}"
    msg = (f"Users: {users_total} • Premium: {users_premium}\n"
           f"Alerts: total={alerts_total}, active={alerts_active}\n"
           f"Subscriptions: total={subs_total} ACTIVE={subs_active} CANCEL_AT_END={subs_cancel} CANCELLED={subs_cancelled} UNKNOWN={subs_unknown}")
    if note: msg += "\nNotes:" + note
    for ch in safe_chunks(msg): await target_msg(update).reply_text(ch)
        ...
        try:
            rows = s.execute(text("""
                SELECT s.id, s.user_id, s.provider, s.status_internal,
                       COALESCE(s.provider_sub_id,'') provider_ref,
                       s.created_at, u.telegram_id
                FROM subscriptions s
                LEFT JOIN users u ON u.id = s.user_id
                ORDER BY s.id DESC LIMIT 20
            """)).all()
        except Exception as e:
            await target_msg(update).reply_text(f"query error: {e}"); return
    if not rows: await target_msg(update).reply_text("No subscriptions in DB."); return
    lines = [f"#{r.id} uid={r.user_id or '-'} tg={r.telegram_id or '-'} {r.status_internal or '-'} ref={r.provider_ref or '-'} created={r.created_at or '-'}" for r in rows]
    for ch in safe_chunks("Last 20 subscriptions:\n" + "\n".join(lines)): await target_msg(update).reply_text(ch)

async def cmd_admincheck(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if _require_admin(update): await target_msg(update).reply_text("Admins only."); return
    try:
        try: url_masked = engine.url.render_as_string(hide_password=True)
        except Exception: url_masked = str(engine.url)
        with session_scope() as s:
            total = s.execute(text("SELECT COUNT(*) FROM alerts")).scalar_one()
            rows = s.execute(text("""
        ...
        for r in rows:
            op = op_from_rule(r.rule)
            lines.append(f"#{r.id} uid={r.user_id} tg={r.telegram_id or '-'} {r.symbol} {op} {r.value} {'ON' if r.enabled else 'OFF'} fired={r.last_fired_at or '-'} met={r.last_met}")
        for ch in safe_chunks("\n".join(lines) if rows else "\n".join(lines+['(none)'])): await target_msg(update).reply_text(ch)
    except Exception as e:
...
async def cmd_testalert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        r = requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                          json={"chat_id": tg_id, "text": "Test alert ✅"}, timeout=10)
...
async def cmd_resetalert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if _require_admin(update): await target_msg(update).reply_text("Admins only."); return
    if not context.args: await target_msg(update).reply_text("Usage: /resetalert <id>"); return
    try: aid = int(context.args[0])
    except Exception: await target_msg(update).reply_text("Bad id"); return
    with session_scope() as s:
        row = s.execute(text("SELECT id FROM alerts WHERE id=:id"), {"id": aid}).first()
        if not row: await target_msg(update).reply_text(f"Alert {aid} not found"); return
        s.execute(text("UPDATE alerts SET last_fired_at=NULL, last_met=FALSE WHERE id=:id"), {"id": aid})
        s.commit()
    await target_msg(update).reply_text(f"Alert (ID {aid}) reset.")

async def cmd_runalerts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if _require_admin(update): await target_msg(update).reply_text("Admins only."); return
    with session_scope() as s:
        counters = run_alert_cycle(s)
        rows = s.execute(text("SELECT id, symbol, rule, value, enabled, last_fired_at, last_met FROM alerts ORDER BY id DESC LIMIT 5")).all()
...
# ── Callbacks (incl. ack:keep/ack:del) ──
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ack buttons on fired alert card
    if data.startswith("ack:"):
        ...
        if action == "keep":
            ...
            except Exception: await query.answer("Kept.")
            return
        if action == "del":
            with session_scope() as s:
                owner = s.execute(text("SELECT user_id FROM alerts WHERE id=:id"), {"id": aid}).first()
            ...
            except Exception: await query.answer("Deleted.")
            return
...
def alerts_loop():
    ...
    got = lock_conn.execute(text("SELECT pg_try_advisory_lock(:id)"), {"id": int(os.getenv("ALERTS_LOCK_ID","911002"))}).scalar()
    if not got: print({"msg": "alerts_lock_skipped"}); lock_conn.close(); return
...
def run_bot():
    ...
    lock_conn = engine.connect()
    got = lock_conn.execute(text("SELECT pg_try_advisory_lock(:id)"), {"id": int(os.getenv("BOT_LOCK_ID","911001"))}).scalar()
    if not got: print({"msg": "bot_lock_skipped"}); lock_conn.close(); return
    try:
